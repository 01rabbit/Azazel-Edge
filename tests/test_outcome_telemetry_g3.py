...
class FakeSource:
    ...
    def sample(self, *, interface: str, epoch: float) -> TelemetrySample:
        ...
        return TelemetrySample(
            ...
            coverage={"proc_net_dev": True, "proc_loadavg": True, "proc_meminfo": True, "proc_uptime": True},
...
def rust_event(*, ts: str, trace: str, src_ip: str = "10.0.0.8", action: str = "throttle") -> dict:
    return {
        "normalized": {
            "ts": ts,
            "src_ip": src_ip,
        ...
        "enforcement": {
            "mode": "enforced" if action == "throttle" else "disabled",
            "trace_id": trace,
            ...
            ] if action == "throttle" else [],
            "rollback_plan": ["tc qdisc del dev br0 root handle 1a:"] if action == "throttle" else [],
            "executed_count": 1 if action == "throttle" else 0,
            "failed_count": 0,
            "errors": [],
            "result": "applied" if action == "throttle" else "no_disruptive_action",
            "metadata": {
...
class OutcomeTelemetryPolicyTests(unittest.TestCase):
    ...
    def test_policy_rejects_unsafe_or_unbounded_window(self) -> None:
...
class OutcomeTelemetryCollectorTests(unittest.TestCase):
    ...
    def test_freezes_real_prebuffer_and_collects_post_window_without_claiming_effect(self) -> None:
        collector = self._collector()
        for epoch in (997.0, 998.0, 999.0):
            collector.record_sample(epoch=epoch)
        collector.observe_event(
            rust_event(ts="1970-01-01T00:16:39+00:00", trace="pre-a"),
            observed_epoch=999.2,
        )
        started = collector.observe_event(
        ...
        for epoch in (1001.0, 1002.0, 1003.0):
            collector.record_sample(epoch=epoch)
        collector.observe_event(
            rust_event(ts="1970-01-01T00:16:41+00:00", trace="post-a"),
        ...
        self.assertGreaterEqual(outcome.baseline_metrics["telemetry_sample_count"], 2)
        self.assertGreaterEqual(outcome.post_metrics["telemetry_sample_count"], 2)
    ...
    def test_no_policy_for_action_means_no_observation(self) -> None:
        collector = self._collector()
        event = rust_event(ts="1970-01-01T00:16:40+00:00", trace="redirect-a", action="redirect")
        event["enforcement"]["mode"] = "enforced"
        event["enforcement"]["result"] = "applied"
        event["enforcement"]["executed_count"] = 1
        event["enforcement"]["command_plan"] = [
            "nft insert rule inet azazel_edge prerouting ip saddr 10.0.0.8 tcp dport 22 redirect to 12222 comment azazel-edge:release-redirect-a"
        ]
        event["enforcement"]["rollback_plan"] = [
            "nft delete rule inet azazel_edge prerouting handle <owned:azazel-edge:release-redirect-a>"
        ]
        event["enforcement"]["metadata"].update(
            {
                "release_task_id": "release-redirect-a",
                "release_resource_key": "nft:inet:azazel_edge:prerouting:10.0.0.8:tcp:22",
                "release_owner_token": "azazel-edge:release-redirect-a",
                "release_tc_handle": "",
            }
        )
        self.assertFalse(collector.observe_event(event, observed_epoch=1000.0))
        self.assertEqual(collector.pending, {})

    def test_release_truncates_post_window_but_never_marks_effective(self) -> None:
        ...
        self.assertTrue(collector.observe_event(release_event(trace="decision-r", attempted_epoch=1002.0), observed_epoch=1002.0))
        collector.record_sample(epoch=1002.0)
        records = collector.finalize_due(epoch=1002.1)
        self.assertEqual(len(records), 1)
        outcome = records[0]
    ...
    def test_collector_started_too_late_reports_baseline_gap(self) -> None:
        ...
        collector.record_sample(epoch=1001.0)
        collector.record_sample(epoch=1002.0)
        collector.record_sample(epoch=1003.0)
    ...
    def test_pending_capacity_drops_evidence_request_without_affecting_control(self) -> None:
        collector = self._collector(max_pending=1)
        self.assertTrue(
            collector.observe_event(
                rust_event(ts="1970-01-01T00:16:40+00:00", trace="one"),
                observed_epoch=1000.0,
            )
        )
        self.assertFalse(
    ...
    def test_duplicate_execution_does_not_create_second_pending_observation(self) -> None:
        collector = self._collector()
        event = rust_event(ts="1970-01-01T00:16:40+00:00", trace="same")
        self.assertTrue(collector.observe_event(event, observed_epoch=1000.0))
        self.assertFalse(collector.observe_event(event, observed_epoch=1000.1))
        self.assertEqual(len(collector.pending), 1)

    def test_output_is_stable_for_same_execution_and_window(self) -> None:
        collector = self._collector()
        for epoch in (997.0, 998.0, 999.0):
            collector.record_sample(epoch=epoch)
        collector.observe_event(
        ...
        for epoch in (1001.0, 1002.0, 1003.0):
            collector.record_sample(epoch=epoch)
        first = collector.finalize_due(epoch=1003.1)[0]
        self.assertTrue(first.outcome_id.startswith("outcome-"))
        self.assertEqual(first.observation_window["action_observed_epoch"], 1000.0)
