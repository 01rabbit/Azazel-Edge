...
class DashboardDataContractTests(unittest.TestCase):
    ...
    def test_dashboard_summary_contract(self) -> None:
        ...
        self.assertAlmostEqual(payload["command_strip"]["ai_contribution_rate"], 0.75)
        self.assertAlmostEqual(payload["command_strip"]["ai_fallback_rate"], 0.25)
    ...
    def test_dashboard_evidence_contract(self) -> None:
        ...
        self.assertIn("alert_aggregation", payload)
        self.assertIn("groups", payload["alert_aggregation"])
        self.assertIn("escalation_queue", payload["alert_aggregation"])
    ...
    def test_dashboard_index_renders_new_sections(self) -> None:
        ...
        # SENTINEL decision console drives view state off the body's
        # data-audience mode (see static/sentinel.js), so the audience toggle
        # is part of the current contract.
        self.assertIn("data-audience", text)
        self.assertIn('id="audienceProfessional"', text)
        self.assertIn('id="audienceTemporary"', text)
        ...
        self.assertIn('id="temporaryMissionHeadline"', text)
        ...
        self.assertIn('id="stripAiContribution"', text)
        self.assertIn('id="stripAiFallback"', text)
        ...
        self.assertIn('id="mioAskForm"', text)
        ...
        self.assertIn('id="askMioLink"', text)
    ...
    def test_dashboard_index_renders_japanese_labels_when_requested(self) -> None:
        ...
        # The SENTINEL console replaced the old workspace switcher (which
        # surfaced the 総合 / シンプル labels) with the tier-nav layout, so those
        # labels are no longer emitted; 表示言語 remains the localized language
        # control label. Operational headings stay English by design.
